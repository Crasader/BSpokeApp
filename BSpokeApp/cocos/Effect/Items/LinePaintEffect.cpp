...
LinePaintEffect::LinePaintEffect():
m_testRenderTexture(NULL){
    m_effectType = EffectType_LinePaintEffect;
}
...
LinePaintEffect* LinePaintEffect::create(){
    LinePaintEffect* ret = new LinePaintEffect();
    if(ret && ret->init()){
        ret->autorelease();
        return ret;
    }
...
bool LinePaintEffect::init(){
    m_shaderName = "LinePaintShader";
    
    GLProgram* program = GLProgramCache::getInstance()->getGLProgram(m_shaderName);
    
    if(!program){
        program = GLProgram::createWithFilenames("shader/base_pos_tex.vert", "shader/base_alpha_color.frag");
...
void LinePaintEffect::applyToSprite(Sprite* sprite){
    
    //    m_runningPaintItem.resize(BATCH_NODE_CAPACITY);
    //    applyOriginEffect();
}

void LinePaintEffect::applyToSpriteWithParent(Sprite* sprite, Node* parent){
    ...
    std::string texturePath = "effect/pattern6.png";
    std::string plistPath   = "effect/pattern6.plist";
    ...
    m_renderTexture->setGLProgram(GLProgramCache::getInstance()->getGLProgram(m_shaderName));
...
void LinePaintEffect::cancelToSprite(Sprite* sprite){
    m_renderTexture->removeFromParent();
}

void LinePaintEffect::runAction(const Vec2& delta){
...
void LinePaintEffect::testColorArea(Sprite* sprite){
    ...
    m_testTextureFinishCommand.func = CC_CALLBACK_0(LinePaintEffect::onTestColorAreaCallback, this);
    
    Director::getInstance()->getRenderer()->addCommand(&m_testTextureFinishCommand);
}

//找周围9个方向都有像素的点
void LinePaintEffect::onTestColorAreaCallback(){
    ...
    for(int i = 0; i < height; i++){
        ...
        for(int j = 0; j < width; j++){
            ...
            if(!hasAlphaPixel && hasNotAlphyPixel){
                ...
                insidePoint.x = j - width / 2;
                insidePoint.y = -(i - height / 2);
                
                m_insidePointVec.push_back(insidePoint);
...
LinePaintEffect::PaintItem LinePaintEffect::generatePaintItem(const Vec2 dir){
    LinePaintEffect::PaintItem ret;
...
//    ret.rotate = CC_RADIANS_TO_DEGREES(dir.getAngle(Vec2(1, 0)));
    ...
    //    Vec2 posititon(0,0);
    Vec2 posititon = m_insidePointVec[random(0, (int)m_insidePointVec.size() - 1)];
...
bool LinePaintEffect::applyAnimate(PaintItem& paintItem, float dt){
